...
export async function planner(ns, target, host)
{
  ...
  for (let i = 0; i < maxThreads; ++i) {
    ...
    hack(ns, { server: cp, threads: i, player });
    let growThreads;
    for (growThreads = 0; growThreads < 10000; ++growThreads) {
      let cp2 = Object.assign({}, cp);
      grow(ns, { server: cp2, threads: growThreads, player, host });
      let newMoney = cp.moneyAvailable;
    ...
    let weakenThreads = (cp.hackDifficulty - cp.minDifficulty) /
