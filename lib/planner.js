...
export async function evaluatePlan(ns, { target, player, host, plan })
{
  ...
  for (const { action, threads } of plan) {
    plan[action](ns, { server: target, host, threads, player });
  }
  return {
    target,
    player
...
export async function planner(ns, target, host)
{
  ...
  for (let i = 0; i < maxThreads; ++i) {
    let moneyShrink = formulas.calculatePercentMoneyHacked(ns, targetServer, player);
    let moneyGrowth = formulas.calculateServerGrowth(ns, targetServer, i, player, host.cpuCores);
    result.push({
