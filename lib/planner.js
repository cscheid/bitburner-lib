...
export async function planBootstrap(ns, target, host, budget)
{
  ...
  let growthAmount = targetServer.moneyMax / Math.min(targetServer.moneyAvailable, 1);
  ...
  if (neededGrowthCycles + neededWeakenCycles > budget) {
    ...
    if (neededWeakenCycles > 0) {
      weaken(ns, { server: targetServer, host, budget });
      return [{
        weaken: budget
      ...
      grow(ns, { targetServer, host, growAmount });
      weaken(ns, { targetServer, host, weakenAmount });
      return [{
        grow: growAmount,
      }, {
        weaken: weakenAmount
