...
)
{
  ...
  const maxSecurity = sH * node.minSecurity;
  
  const mFraction = node.money / node.maxMoney;
  const sFraction = node.security / maxSecurity;
  ...
  const mU = clamp(delerp(mFraction, moneyT), [0, 1]);
  const sU = clamp(delerp(sFraction, securityT), [0, 1]);
  ...
  function setLog(action) {
    ...
    port.write({
      host,
      what: action,
      threads: ns.getRunningScript().threads
    });
  }
