...
// function simulateHack(ns, node, threads)
// {
//   const numServerGrowthCycles = Math.max(Math.floor(threads), 0);

//   //Get adjusted growth rate, which accounts for server security
//   const growthRate = 1.03;
//   let adjGrowthRate = 1 + (growthRate - 1) / node.difficulty;
...
//   return Math.pow(adjGrowthRate, numServerGrowthCyclesAdjusted);
  
// }

// function simulateGrow(ns, node, threads)
// {
  
// }

// function simulateWeaken(ns, node, threads)
// {
  
// }

// export async function simulateNode(ns, node)
// {
//   const ss = getSharedState(ns, "monitor-loic-queue");
//   let backoff = 0;
//   while (ss.state === undefined) {
//     await ns.sleep(Math.random() * (1 << (backoff++)));
//   }

//   // nothing there, assume node won't change
//   if (ss.state[node.name] === undefined) {
//     return node;
//   }
...
//     hack: simulateHack,
//     grow: simulateGrow,
//     weaken: simulateWeaken
//   };
//   for (const { what, threads } of ss.state[node.name]) {
//     node = dispatch(what)(ns, node, threads);
//   }
// };
...
)
{
  ...
  const ss = getSharedState(ns, "loic-params");
  if (ss.weights === undefined) {
    ss.weights = [130, 7, 11]; // good weights for loicing n00dles
  }
  
  switch (choose(ss.weights)) {
  case 0:
    await logNow(ns);
